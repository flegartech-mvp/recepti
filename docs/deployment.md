...
- Node.js 20 LTS or newer and pnpm.
...
The eight migrations under `supabase/migrations/` are authoritative. Apply
them in timestamp order through
`202607150008_strict_owner_gate.sql`. Do not reproduce them manually
...
- [ ] `pnpm format:check`, `lint`, `typecheck`, `test`, `test:e2e`, and `build`
      pass from the release commit.
- [ ] Local `db reset --local` and `pnpm test:db` pass.
- [ ] Hosted migration history contains migrations 001 through 008.
...
Run `pnpm dlx supabase migration list` against the linked project. Apply every
migration through 008 before deploying the matching application.
...
Start this project's stack with `pnpm dlx supabase start`. Stop other local
