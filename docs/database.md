...
A small set of internal Auth/statistics/cover/staple trigger functions is
...
authenticated application owner inspect the `recipe-images` bucket metadata.
The RPC reports safe booleans for required tables/RPCs, migration-dependent
contracts, RLS, and that private image bucket. It returns no configured email,
token, row data, or Storage object name, and is not executable by `public` or
`anon`.
