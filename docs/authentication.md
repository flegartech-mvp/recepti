...
sequenceDiagram
  ...
  Browser->>Nana: Continue with Google
...
key printed by `pnpm dlx supabase status`. Keep `NEXT_PUBLIC_SITE_URL` on the
...
the process is not production. The cookie `menta-e2e-role` may represent
