...
- Node.js 22 LTS or newer (matching `package.json`).
...
to `main` and pull requests. Browser tests use only the local guarded test
identity. Database integration starts an isolated local Supabase stack, applies
the complete migration chain, lints SQL, verifies owner persistence and
non-owner rejection, and stops the stack afterward. No production secret is
required or exposed to forked pull requests.
